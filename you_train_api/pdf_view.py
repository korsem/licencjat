...
class CyclicTrainingPlanPDF(BaseTrainingPlanPDF):
    # dodaj do tego co aktualnie jest w header plan.start_date i plan.end_date jako start_date + relativetimedelta(weeks=cycle_length)
    # i informacje na ile tygodni jest plan tzn cycle_length
    def add_workouts(self):
        workouts = WorkoutInPlan.objects.filter(
            workout_plan__training_plan=self.training_plan
        ).order_by(
            "day_of_week"
        )  # powinny być w tabelce
        for workout_in_plan in workouts:
            ...
            self.content.append(
                Paragraph(
                    f"Date: {get_polish_day_of_week(workout_in_plan.day_of_week)}",
                    self.styles["normal"],
                )
            )
            for segment in workout.segments.all():
                ...
                for exercise in segment.exercises.all():
                    self.content.append(
                        Paragraph(f"Exercise: {exercise.name}", self.styles["Bold"])
                    )
                    self.content.append(Spacer(1, 12))
            self.content.append(Spacer(1, 12))
...
class NonCyclicTrainingPlanPDF(BaseTrainingPlanPDF):
    # dodaj do tego co aktualnie jest w header plan.start_date i plan.end_date jako start_date
    def add_workouts(self):
        workouts = WorkoutInPlan.objects.filter(
            workout_plan__training_plan=self.training_plan
        ).order_by("date")
        for workout_in_plan in workouts:
            ...
            for segment in workout.segments.all():
                ...
                for (
                    ...
                    segment.exercises.all()
                ):  # exercise in plan żeby wyświuetlić powtórzenia lub duration i przerwe?
                    self.content.append(
                        Paragraph(f"Exercise: {exercise.name}", self.styles["Bold"])
                    )
                    self.content.append(Spacer(1, 12))
